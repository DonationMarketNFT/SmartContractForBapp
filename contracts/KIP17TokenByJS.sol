...
contract KIP17Metadata is KIP13, KIP17, IKIP17Metadata {
    ...
    mapping(uint256 => string) private _tokenName; // campaign name
    mapping(uint256 => string) private _tokenDescription; // campaign description
    mapping(uint256 => string) private _tokenIPFS; // ipfs url
    mapping(uint256 => string) private _tokenOwnerName; // campaign 발행인 이른
    mapping(uint256 => string) private _tokenAgencyUrl; // campaign url
    mapping(uint256 => string) private _tokenDate; // funding date
    mapping(uint256 => string) private _tokenNumber; // funding number;
    ...
    function tokenIPFS(uint256 tokenId) external view returns (string memory){
        require(
            _exists(tokenId),
            "KIP17Metadata: URI query for nonexistent token"
        );
        return _tokenIPFS[tokenId];
    }
    ...
    string memory tokenNumber
        ...
        _tokenIPFS[tokenId] = tokenIPFS; // campaign ipfs
        _tokenOwnerName[tokenId] = tokenOwnerName; // campaign 발행인 이름
        _tokenAgencyUrl[tokenId] = tokenAgencyUrl; // campaign url
        _tokenDate[tokenId] = tokenDate; // funding date
        _tokenNumber[tokenId] = tokenNumber; // funding number;
...
contract KIP17MetadataMintable is KIP13, KIP17, KIP17Metadata, MinterRole {
    ...
    function mintWithTokenURI(
        ...
        _mint(to, tokenId);
        _setTokenURI(tokenId, tokenURI, tokenIPFS, tokenOwnerName, tokenAgencyUrl, tokenDate, tokenNumber);
        return true;
    }
}
